...
const AddressForm = ({checkoutToken, next}) => {
    ...
    const countries = Object.entries(shippingCountries).map(
        ([code, nameCountry]) => 
            ({id: code, label: nameCountry})
    )
    const subdivisions = Object.entries(shippingSubdivisions).map(
        ([code, nameSubdivision]) => 
            ({id: code, label: nameSubdivision})
    )
    ...
    return ( 
        <div>
            <Typography variant="h6" className="title-form">Shipping Address</Typography>
            <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit((data) => next({ ...data, shippingCountry, shippingSubdivision, shippingOption}))}>
                   <Grid container spacing={3}>
                        ...
                        <CustomTextField required name='zip' label='ZIP / Postal code / CEP' />
                         <Grid item xs={12} sm={6}>

                            <InputLabel className="title-form">Shipping Country</InputLabel>
                        ...
                        <Grid item xs={12} sm={6}>
                            <InputLabel className="title-form">Shipping Subdivision</InputLabel>
                        ...
                        <Grid item xs={12} sm={6}>
                            <InputLabel className="title-form">Shipping Options</InputLabel>
